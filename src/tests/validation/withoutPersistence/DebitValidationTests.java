	...
	private void testDebitOK() {
		...
		try {
			...
			if (Integer.parseInt(infos.get(4)) != 300)
				System.out
						.println("le nombre de jetons cr�dit�s est incorrect (300 != "
								+ infos.get(4) + ")");
		} catch (Exception e) {
			System.out
					.println("debit avec upseudo connu, nombre de jetons valide et mdp gestionnaire correct a lev� l'exception "
							+ e.getClass());
		}
		try {
			...
			if (Integer.parseInt(infos.get(4)) != 250)
				System.out
						.println("le nombre de jetons cr�dit�s est incorrect (250 != "
								+ infos.get(4) + ")");
		} catch (Exception e) {

			System.out
					.println("debit avec pseudo connu, nombre de jetons valide et mdp gestionnaire correct a lev� l'exception "
							+ e.getClass());
		}
	}
}